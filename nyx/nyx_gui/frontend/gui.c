...
static void _save_fb_to_bmp()
{
	...
	lv_mbox_set_text(mbox, SYMBOL_CAMERA"  #FFDD00 截图保存中...#");
	...
	if (!res)
		lv_mbox_set_text(mbox, SYMBOL_CAMERA"  #96FF00 截图保存成功!#");
	else
...
lv_obj_t *nyx_create_standard_window(const char *win_title)
{
	...
	close_btn = lv_win_add_btn(win, NULL, SYMBOL_CLOSE" 关闭", nyx_win_close_action_custom);
...
lv_obj_t *nyx_create_window_custom_close_btn(const char *win_title, lv_action_t rel_action)
{
	...
	close_btn = lv_win_add_btn(win, NULL, SYMBOL_CLOSE" 关闭", rel_action);
...
static void _check_sd_card_removed(void *params)
{
	...
	if (!do_auto_reload && sd_get_card_removed())
	{
		...
		static const char * mbox_btn_map[] = { "\221重启(RCM)", "\221关机", "\221不要重新加载", "" };
		static const char * mbox_btn_map_rcm_patched[] = { "\221重启", "\221关机", "\221不要重新加载", "" };
		lv_obj_t *mbox = lv_mbox_create(dark_bg, NULL);
		lv_mbox_set_recolor_text(mbox, true);
