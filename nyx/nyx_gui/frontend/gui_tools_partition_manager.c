...
static lv_res_t _action_flash_android_data(lv_obj_t * btns, const char * txt)
{
	...
	if (offset_sct && size_sct)
	{
		...
		if ((file_size >> 9) > size_sct)
			s_printf(txt_buf, "#FF8000 警告:# 内核镜像太大!\n");
		else
		{
...
recovery_not_found:
	...
	if (offset_sct && size_sct)
	{
		...
		if ((file_size >> 9) > size_sct)
			strcat(txt_buf, "#FF8000 警告:# DTB镜像太大!");
		else
		{
			sdmmc_storage_write(part_info.storage, offset_sct, file_size >> 9, buf);
...
static lv_res_t _sd_create_mbox_start_partitioning()
{
	...
	static const char *mbox_btn_map3[] = { "\222SD UMS", "\222F刷写 Android", "\221确定", "" };
	lv_obj_t *mbox = lv_mbox_create(dark_bg, NULL);
	lv_mbox_set_recolor_text(mbox, true);
	lv_obj_set_width(mbox, LV_HOR_RES / 9 * 6);

	lv_mbox_set_text(mbox, "#FF8000 SD分区管理器#");
	lv_obj_align(mbox, NULL, LV_ALIGN_CENTER, 0, 0);
	lv_obj_set_top(mbox, true);
...
static lv_res_t _create_mbox_partitioning_warn()
{
	...
	static const char *mbox_btn_map[] = { "\222SD UMS", "\222开始", "\222取消", "" };
	static const char *mbox_btn_map2[] = { "\222开始", "\222取消", "" };
	lv_obj_t * mbox = lv_mbox_create(dark_bg, NULL);
	lv_mbox_set_recolor_text(mbox, true);
	...
	if (!part_info.emmc)
	{
		...
		if (part_info.backup_possible)
		{
			strcat(txt_buf, "#C7EA46 您的文件将被备份和恢复!#\n"
				"#FFDD00 任何其他分区都将被擦除!#");
		}
		else
		{
...
lv_res_t create_window_partition_manager(bool emmc)
{
	...
	if (!emmc)
	{
		win = nyx_create_standard_window(SYMBOL_SD" SD分区管理器");
		lv_win_add_btn(win, NULL, SYMBOL_MODULES_ALT" 修复混合MBR/GPT", _action_fix_mbr_gpt);
	}
	else
		win = nyx_create_standard_window(SYMBOL_CHIP" eMMC分区管理器");
	...
	lv_label_set_static_text(label_btn, SYMBOL_DOWNLOAD"  刷写Linux");
	if (!emmc)
